package com.domencai.one.particleview;
...
public class ParticleView extends View {
    ...
    private void initView(AttributeSet attrs) {
        ...
        mHostText = null == typeArray.getString(R.styleable.ParticleView_pv_host_text) ? "" :
                typeArray.getString(R.styleable.ParticleView_pv_host_text);
        mParticleText = null == typeArray.getString(R.styleable.ParticleView_pv_particle_text) ?
                "" : typeArray.getString(R.styleable.ParticleView_pv_particle_text);
        mParticleTextSize = (int) typeArray.getDimension(R.styleable
                .ParticleView_pv_particle_text_size, DEFAULT_MIN_TEXT_SIZE);
        int hostTextSize = (int) typeArray.getDimension(R.styleable
                .ParticleView_pv_host_text_size, DEFAULT_MIN_TEXT_SIZE);
        ...
        mTextAnimTime = typeArray.getInt(R.styleable.ParticleView_pv_text_anim_time,
                DEFAULT_TEXT_ANIM_TIME);
        mSpreadAnimTime = typeArray.getInt(R.styleable.ParticleView_pv_text_anim_time,
                DEFAULT_SPREAD_ANIM_TIME);
        mHostTextAnimTime = typeArray.getInt(R.styleable.ParticleView_pv_text_anim_time,
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mStartMinP = new PointF(mWidth / 2 - getTextWidth(mParticleText, mParticleTextPaint) / 2f
                - dip2px(4), mHeight / 2 + getTextHeight(mHostText, mHostTextPaint) / 2 -
                getTextHeight(mParticleText, mParticleTextPaint) / 0.7f);
        mEndMinP = new PointF(mWidth / 2 + getTextWidth(mParticleText, mParticleTextPaint) / 2f +
                dip2px(10), mHeight / 2 + getTextHeight(mHostText, mHostTextPaint) / 2);

        for (int i = 0; i < ROW_NUM; i++) {
            for (int j = 0; j < COLUMN_NUM; j++) {
                mMinParticles[i][j] = new Particle(mStartMinP.x + (mEndMinP.x - mStartMinP.x) /
                        COLUMN_NUM * j, mStartMinP.y + (mEndMinP.y - mStartMinP.y) / ROW_NUM * i,
                        dip2px(0.8f));
            }
        }

        mStartMaxP = new PointF(mWidth / 2 - DEFAULT_MAX_TEXT_SIZE, mHeight / 2 -
                DEFAULT_MAX_TEXT_SIZE);
        mEndMaxP = new PointF(mWidth / 2 + DEFAULT_MAX_TEXT_SIZE, mHeight / 2 +
                DEFAULT_MAX_TEXT_SIZE);

        for (int i = 0; i < ROW_NUM; i++) {
            for (int j = 0; j < COLUMN_NUM; j++) {
                mParticles[i][j] = new Particle(mStartMaxP.x + (mEndMaxP.x - mStartMaxP.x) /
                        COLUMN_NUM * j, mStartMaxP.y + (mEndMaxP.y - mStartMaxP.y) / ROW_NUM * i,
                        getTextWidth(mHostText + mParticleText, mParticleTextPaint) / (COLUMN_NUM
                                                                                               *
                                                                                               1.8f));
            }
        }

        Shader linearGradient = new LinearGradient(mWidth / 2 - getTextWidth(mParticleText,
                mCirclePaint) / 2f, mHeight / 2 - getTextHeight(mParticleText, mCirclePaint) / 2,
                mWidth / 2 - getTextWidth(mParticleText, mCirclePaint) / 2, mHeight / 2 +
                getTextHeight(mParticleText, mCirclePaint) / 2, new int[]{mParticleColor, Color
                .argb(120, getR(mParticleColor), getG(mParticleColor), getB(mParticleColor))},
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mStatus == STATUS_PARTICLE_GATHER) {
            for (int i = 0; i < ROW_NUM; i++) {
                for (int j = 0; j < COLUMN_NUM; j++) {
                    canvas.drawCircle(mParticles[i][j].x, mParticles[i][j].y, mParticles[i][j]
                            .radius, mCirclePaint);
                }
            }
        }
        ...
        if (mStatus == STATUS_TEXT_MOVING) {
            canvas.drawText(mHostText, mHostTextX, mHeight / 2 + getTextHeight(mHostText,
                    mHostBgPaint) / 2, mHostBgPaint);
            canvas.drawRect(mHostTextX + mHostRectWidth, mHeight / 2 - getTextHeight(mHostText,
                    mHostBgPaint) / 1.2f, mHostTextX + getTextWidth(mHostText, mHostTextPaint),
                    mHeight / 2 + getTextHeight(mHostText, mHostBgPaint) / 1.2f, mHostTextPaint);
        }

        if (mStatus == STATUS_PARTICLE_GATHER) {
            canvas.drawRoundRect(new RectF(mWidth / 2 - mSpreadWidth, mStartMinP.y, mWidth / 2 +
                    mSpreadWidth, mEndMinP.y), dip2px(2), dip2px(2), mHostBgPaint);
            canvas.drawText(mParticleText, mWidth / 2 - getTextWidth(mParticleText,
                    mParticleTextPaint) / 2, mStartMinP.y + (mEndMinP.y - mStartMinP.y) / 2 +
                    getTextHeight(mParticleText, mParticleTextPaint) / 2, mParticleTextPaint);
        } else if (mStatus == STATUS_TEXT_MOVING) {
            canvas.drawRoundRect(new RectF(mParticleTextX - dip2px(4), mStartMinP.y,
                    mParticleTextX + getTextWidth(mParticleText, mParticleTextPaint) + dip2px(4),
                    mEndMinP.y), dip2px(2), dip2px(2), mHostBgPaint);
            canvas.drawText(mParticleText, mParticleTextX, mStartMinP.y + (mEndMinP.y -
                                                                                   mStartMinP.y)
                    / 2 + getTextHeight(mParticleText, mParticleTextPaint) / 2, mParticleTextPaint);
        }
    ...
    private void startParticleAnim() {
        ...
        for (int i = 0; i < ROW_NUM; i++) {
            for (int j = 0; j < COLUMN_NUM; j++) {
                final int tempI = i;
                final int tempJ = j;
                ValueAnimator animator = ValueAnimator.ofObject(new LineEvaluator(),
                        mParticles[i][j], mMinParticles[i][j]);
    ...
    private void startSpreadAnim() {
        ValueAnimator animator = ValueAnimator.ofFloat(0, getTextWidth(mParticleText,
                mParticleTextPaint) / 2 + dip2px(4));
    ...
    private void startHostTextAnim() {
        ...
        ValueAnimator particleTextXAnim = ValueAnimator.ofFloat(mStartMinP.x + dip2px(4), mWidth
                / 2 - (getTextWidth(mHostText, mHostTextPaint) + getTextWidth(mParticleText,
                mParticleTextPaint)) / 2 + getTextWidth(mHostText, mHostTextPaint));
        ...
        ValueAnimator hostTextXAnim = ValueAnimator.ofFloat(mStartMinP.x, mWidth / 2 -
                (getTextWidth(mHostText, mHostTextPaint) + getTextWidth(mParticleText,
                        mParticleTextPaint) + dip2px(20)) / 2);
