 ...
 */
package com.domencai.puzzle.visualizer.renderer;
...
import com.domencai.puzzle.visualizer.AudioData;
import com.domencai.puzzle.visualizer.FFTData;
...
public class CircleBarRenderer extends Renderer {
    ...
    public void onRender(Canvas canvas, FFTData data, Rect rect) {
        ...
        for (int i = 0; i < data.bytes.length / mDivisions; i++) {
            ...
            float[] cartPoint = {(float) (i * mDivisions) / (data.bytes.length - 1), rect.height
                    () / 2 - dbValue / 4};
            ...
            float[] cartPoint2 = {(float) (i * mDivisions) / (data.bytes.length - 1), rect.height
                    () / 2 + dbValue};
    ...
    private float[] toPolar(float[] cartesian, Rect rect) {
        ...
        float[] out = {(float) (cX + radius * Math.sin(angle + angleModulation)), (float) (cY + radius * Math.cos(angle + angleModulation))};
        return out;
    }
